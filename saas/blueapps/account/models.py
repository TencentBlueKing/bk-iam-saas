...
class User(AbstractBaseUser, PermissionsMixin):
        ...
        validators=[
            validators.RegexValidator(
                ...
                _('Enter a valid openid. '
                  ...
                  'numbers and underlined characters.'), 'invalid'),
        ],
        error_messages={
    ...
    def send_sms(self, code):
        ...
        try:
            result = sms.send_sms([self.username],
                                  SV_CONF['SMS_FORMAT'].format(code))
        except Exception:  # pylint: disable=broad-except
            logger.error(u'cmsi.send_sms_for_external_user failed. '
                         'username->[%s], code->[%s] for->[%s]' %
                         (self.username, code, traceback.format_exc()))
            return {'result': False, 'message': u'ESB发送短信接口错误，可能由权限问题导致'}
    ...
    def send_code(self):
        ...
        if v_info_cnt == 0:
            ...
            logger.error(u'found more than one code of the user->[%s]' %
                         self.id)
    ...
    def verify_code(self, code):
        check = VerifyInfo.objects.filter(
            ...
            datetime.timedelta(minutes=SV_CONF['VALID_MINUTES'])).count()
        if check == 1:
            # 一个验证码只能用一次 用完删除
