...
class Command(TemplateCommand):
    ...
    def handle(self, **options):
        ...
        for root, dirs, files in os.walk(template_dir):
            ...
            for dirname in dirs[:]:
                if dirname.startswith('.') or dirname == '__pycache__' or (
        ...
        test_component_base = path.join(top_dir, 'blueapps_example',
                                        'test_component')
        test_app_tags_base = path.join(top_dir, 'blueapps_example',
                                       'test_app_tags')
        if run_ver == 'open':
            os.remove(path.join(test_component_base, 'views.py'))
            shutil.move(path.join(test_component_base, 'views_open.py'),
                        path.join(test_component_base, 'views.py'))
            os.remove(path.join(test_app_tags_base, 'views.py'))
            shutil.move(path.join(test_app_tags_base, 'views_open.py'),
        ...
        shutil.move(
            path.join(top_dir, 'blueapps_example', 'static',
                      'blueapps_example'), path.join(top_dir, 'static'))
...
def modify_default_file(old_file_object):
    ...
    with open(
            "%s/conf/example_template/config/default.json" %
            blueapps.__path__[0], 'r') as json_file:
            ...
            for replace_property in json_dict:
                ...
                if propertys.get('mode') == 'add':
                    ...
                    result_content = ''.join([
                        ...
                        str(content), result_content[end_index:]
                    ])
                # mode 为 cover 进行覆盖内容
                    ...
                    result_content = ''.join([
                        ...
                        str(content), result_content[end_index + 5:]
                    ])
                else:
                    # 其他情况
                    break
