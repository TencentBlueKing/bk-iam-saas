...
from backend.biz.action import ActionBean, ActionCheckBiz, ActionForCheck
...
from backend.service.models import ApplyAction
from tests.test_util.factory import ActionFactory


class ActionServiceFillSystemTests(TestCase):
    def test_apply_instance(self):
                ...
                "related_resource_types": [
                    {
                        "system_id": "bk_cmdb",
                        "type": "host",
                        "instances": [[{"system_id": "", "type": "host", "id": "host1", "name": "host1"}]],
                        "attributes": [],
                    }
                ],
            }
        ...
        svc = ActionService()
        action_list = ActionList(svc.list(system_id))
        svc_action = action_list.get(action.id)
        action.fill_instance_system(svc_action)
                    ...
                    "related_resource_types": [
                        {
                            "system_id": "bk_cmdb",
                            "type": "host",
                            "instances": [[{"system_id": "bk_cmdb", "type": "host", "id": "host1", "name": "host1"}]],
                            "attributes": [],
                        }
                    ],
                }
...
class ActionServiceTests(TestCase):
    def test_check_system_actions(self):
        from backend.component import iam
        ...
        iam.list_resource_type = mock.Mock(return_value={})

        action = ActionFactory().example()

        action_check_biz = ActionCheckBiz()

        action_check_biz.check("system", [ActionForCheck.parse_obj(action)])

        with self.assertRaises(Exception):
            action.related_resource_types[0].id = "test"
            action_check_biz.check("system", [ActionForCheck.parse_obj(action)])
