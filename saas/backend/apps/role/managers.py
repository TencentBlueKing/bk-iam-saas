...
class RoleRelatedObjectManager(models.Manager):
    ...
    def create_object_relation(self, tenant_id: str, role_id: int, object_id: int, object_type: str):
        """创建：角色与其产生的模板或用户组的关联关系"""
        return self.create(tenant_id=tenant_id, role_id=role_id, object_type=object_type, object_id=object_id)

    def batch_create_object_relation(self, tenant_id: str, role_id: int, object_ids: List[int], object_type: str):
        """批量创建：角色与其产生的模板或用户组的关联关系"""
        self.bulk_create(
            [
                self.model(tenant_id=tenant_id, role_id=role_id, object_type=object_type, object_id=object_id)
                for object_id in object_ids
            ],
            batch_size=100,
        )

    def create_template_relation(self, tenant_id: str, role_id: int, template_id: int):
        """
        创建角色与角色创建的模板关系
        """
        return self.create_object_relation(tenant_id, role_id, template_id, RoleRelatedObjectType.TEMPLATE.value)

    def create_subject_template_relation(self, tenant_id: str, role_id: int, template_id: int):
        """
        创建角色与角色创建的人员模板关系
        """
        return self.create_object_relation(
            tenant_id, role_id, template_id, RoleRelatedObjectType.SUBJECT_TEMPLATE.value
        )
    ...
    def create_group_relation(self, tenant_id: str, role_id: int, group_id: int):
        """
        创建角色与角色创建的用户组关系
        """
        return self.create_object_relation(tenant_id, role_id, group_id, RoleRelatedObjectType.GROUP.value)

    def batch_create_group_relation(self, tenant_id: str, role_id: int, group_ids: List[int]):
        """
        批量创建角色与角色创建的用户组关系
        """
        self.batch_create_object_relation(tenant_id, role_id, group_ids, RoleRelatedObjectType.GROUP.value)
