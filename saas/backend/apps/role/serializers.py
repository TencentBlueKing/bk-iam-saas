...
class GradeMangerBaseInfoSLZ(serializers.Serializer):
    ...
    def validate_members(self, value):
        """过滤 members 中不存在的用户，仅保留已存在的"""
        # 子类 (如 ManagementSubsetMangerCreateSLZ) 将 members 覆盖为字符串列表，需要兼容两种格式
        ...
        return [m for m in value if (m["username"] if isinstance(m, dict) else m) in exist_usernames]
