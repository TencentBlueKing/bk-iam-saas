...
class ApprovalProcessGlobalConfig(BaseModel):
    ...
    application_type = models.CharField("申请类型", max_length=32, choices=ApplicationType.get_choices())
    process_id = models.CharField("审批流程 ID", max_length=128)

    class Meta:
        verbose_name = "审批流程全局配置"
        verbose_name_plural = "审批流程全局配置"
        # 每个租户每一种申请类型只能配置一个默认流程
...
class ActionProcessRelation(BaseModel):
    ...
    tenant_id = models.CharField("租户 ID", max_length=64, default="")
