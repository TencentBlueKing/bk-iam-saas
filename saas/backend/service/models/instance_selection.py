...
class PathResourceType(BaseModel):
    """
    用于做视图校验的结构
    !!! 所有需要做视图校验的地方, 都需要先转换成PathResourceType
    """
...
class ChainNode(BaseModel):
    ...
    def match_resource_type(self, resource_type: PathResourceType) -> bool:
        # 兼容无权限申请的system_id可能为空情况
        return self.id == resource_type.id and (
            self.system_id == resource_type.system_id or resource_type.system_id == ""
        )
...
class InstanceSelection(BaseModel):
    ...
    def match_path(self, path: List[PathResourceType]) -> bool:
        ...
        # 严格匹配视图与实例的每个一个节点
        for chain_node, resource_type in zip(self.resource_type_chain, path):
    ...
    def list_match_path_system_id(self, path: List[PathResourceType]) -> List[str]:
        ...
        system_ids = [chain_node.system_id for chain_node, _ in zip(self.resource_type_chain, path)]
