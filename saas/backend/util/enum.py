...
Note: 继承ChoicesEnum的枚举在遍历成员时，将会包括_choices_labels这个枚举项，需要判断去除
目前python3.6不支持 _ignore_，只有>=3.7才支持，引入包aenum来解决这个问题
aenum包完全兼容Python默认的Enum的，https://github.com/ethanfurman/aenum
# from aenum import StrEnum 支持Auto default value is member name
# from aenum import LowerStrEnum 支持Auto default value is member name, lower-cased
"""
from typing import TYPE_CHECKING

from aenum import auto

# 由于mypy无法支持aenum，所以需要在类型检测时使用原始类代替
# https://github.com/ethanfurman/aenum/issues/10#issuecomment-869209856
# TODO: 待blue-krill开源后使用其StructuredEnum进行重构，
#  同时需要搜索所有注释了`# type: ignore[attr-defined]`的地方，若是因为枚举值在定义时使用了ChoicesEnum而导致必须忽略mypy检查的，则删除
if TYPE_CHECKING:
