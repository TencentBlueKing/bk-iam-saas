...
urlpatterns = [
    ...
    # 系统列表 list(不分页，或者分页，page_size 不传默认 100?)
    path(
        "systems/",
    ...
    # 用户的角色列表，list 分页
    # (可以 filter=super/system/grade 来过滤是否分级管理员) 注意这里路径中 user 是 type, 所以不是 users
    path(
    ...
    # 冻结，解冻，查询冻结的用户列表接口
    # (冻结这个用户在蓝鲸平台的所有权限，包括权限中心本身; 注意只能冻结用户，部门和组无法冻结)
    path(
        "freeze/users/",
