...
class ManagementAPIPermission(permissions.IsAuthenticated, ManagementAPIPermissionCheckMixin):
    ...
    def _verify_api(self, view, request, param_source, api):
        ...
        if param_source in [
            ...
            self._verify_api_by_object(
                request.tenant_id,
                app_code,
                VerifyAPIParamSourceToObjectTypeMap[param_source],
                int(object_id),
                api,
                system_id,
            )
        ...
        if param_source == VerifyApiParamLocationEnum.GROUP_IDS_IN_BODY:
            ...
            self._verify_api_by_groups(request.tenant_id, app_code, group_ids, api, system_id)
            return
    ...
    def _verify_api_by_role(self, tenant_id: str, app_code: str, role_id: int, api: ManagementAPIEnum, system_id: str):
        """
        ...
        role_source = RoleSource.objects.filter(
            tenant_id=tenant_id, source_type=RoleSourceType.API.value, role_id=role_id
        ).first()
        ...
        if role_source is None:
            ...
            if (
                ...
                and Role.objects.filter(
                    tenant_id=tenant_id, id=role_id, type=RoleType.SYSTEM_MANAGER.value, code=system_id
                ).exists()
            ):
    ...
    def _verify_api_by_group(
        ...
        # 使用角色校验 API
        self._verify_api_by_role(tenant_id, app_code, role_related_object.role_id, api, system_id)
    ...
    def _verify_api_by_object(
        ...
        if object_type == VerifyAPIObjectTypeEnum.ROLE.value:
            self._verify_api_by_role(tenant_id, app_code, object_id, api, system_id)
        elif object_type == VerifyAPIObjectTypeEnum.GROUP.value:
            self._verify_api_by_group(tenant_id, app_code, object_id, api, system_id)

    def _verify_api_by_groups(
        self, tenant_id: str, app_code: str, group_ids: List[int], api: ManagementAPIEnum, system_id: str
    ):
        """
        ...
        for role_id in role_ids_set:
            self._verify_api_by_role(tenant_id, app_code, role_id, api, system_id)
