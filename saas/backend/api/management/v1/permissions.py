...
class ManagementAPIPermission(permissions.IsAuthenticated, ManagementAPIPermissionCheckMixin):
    ...
    def _verify_api(self, view, request, param_source, api):
        ...
        if param_source in [
            ...
            self._verify_api_by_object(
                app_code, VerifyAPIParamSourceToObjectTypeMap[param_source], int(object_id), api
            )
            return
        ...
        if param_source == VerifyApiParamLocationEnum.GROUP_IDS_IN_BODY:
            ...
            self._verify_api_by_groups(app_code, group_ids, api)
            return

    def _verify_api_by_role(self, app_code: str, role_id: int, api: ManagementAPIEnum):
        """
    ...
    def _verify_api_by_group(self, app_code: str, group_id: int, api: ManagementAPIEnum):
        """
        ...
        # 使用角色校验 API
        self._verify_api_by_role(app_code, role_related_object.role_id, api)
    ...
    def _verify_api_by_object(
        ...
        if object_type == VerifyAPIObjectTypeEnum.ROLE.value:
            self._verify_api_by_role(app_code, object_id, api)
        elif object_type == VerifyAPIObjectTypeEnum.GROUP.value:
            self._verify_api_by_group(app_code, object_id, api)

    def _verify_api_by_groups(self, app_code: str, group_ids: List[int], api: ManagementAPIEnum):
        """
        ...
        for role_id in role_ids_set:
            self._verify_api_by_role(app_code, role_id, api)
