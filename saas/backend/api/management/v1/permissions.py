...
class ManagementAPIPermission(permissions.IsAuthenticated, ManagementAPIPermissionCheckMixin):
    ...
    def _verify_api(self, view, request, param_source, api):
        ...
        if param_source in [
            ...
            # 由于 URL 参数是必然会获取得到的，否则是已经 404 的，
            # 不会执行到这里，所以可以直接 [] 获取而不需要判断是否存在
            object_id = request.parser_context["kwargs"][object_id_key]
            self._verify_api_by_object(
                request.tenant_id, app_code, VerifyAPIParamSourceToObjectTypeMap[param_source], int(object_id), api
            )
            return

        # 参数 system 来自 Body Data 或 Query
        if param_source in [
            VerifyApiParamLocationEnum.SYSTEM_IN_BODY.value,
            VerifyApiParamLocationEnum.SYSTEM_IN_QUERY.value,
        ]:
            # 确定参数是在 body 还是在 query 里
            request_params = (
                request.data
                if param_source == VerifyApiParamLocationEnum.SYSTEM_IN_BODY.value
                else request.query_params
            )
            # 取 request body/param参数值, TODO: 后续考虑是否约定传入所需参数的 Key 列表
            # 序列化保证 system 参数一定存在，这里序列化只解析了需要的，不会将其他的进行解析
        ...
        if param_source == VerifyApiParamLocationEnum.GROUP_IDS_IN_BODY:
            ...
            self._verify_api_by_groups(request.tenant_id, app_code, group_ids, api)
            return

    def _verify_api_by_role(self, tenant_id: str, app_code: str, role_id: int, api: ManagementAPIEnum):
        """
        由于管理员 API 有很大一部分都是在需要在分级管理员角色下操作的
        所以该方法主要是封装了对角色下的 API 校验
        """
        # 查询分级管理员的来源系统
        role_source = RoleSource.objects.filter(
            source_type=RoleSourceType.API.value, role_id=role_id, tenant_id=tenant_id
        ).first()
        ...
        if role_source is None:
            raise exceptions.PermissionDenied(
                ...
                f"since role is not created by app_code[{app_code}]."
            )
    ...
    def _verify_api_by_group(self, tenant_id: str, app_code: str, group_id: int, api: ManagementAPIEnum):
        ...
        role_related_object = RoleRelatedObject.objects.filter(
            tenant_id=tenant_id, object_type=RoleRelatedObjectType.GROUP.value, object_id=group_id
        ).first()
        # 查询不到用户组对应的角色，说明无权限访问
        if role_related_object is None:
            raise exceptions.PermissionDenied(
                detail=f"group[{group_id}] can not operated by app_code[{app_code}], "
                f"since related role of group is not created by app_code[{app_code})]."
            )
        # 使用角色校验 API
        self._verify_api_by_role(tenant_id, app_code, role_related_object.role_id, api)
    ...
    def _verify_api_by_object(
        ...
        if object_type == VerifyAPIObjectTypeEnum.ROLE.value:
            self._verify_api_by_role(tenant_id, app_code, object_id, api)
        elif object_type == VerifyAPIObjectTypeEnum.GROUP.value:
            self._verify_api_by_group(tenant_id, app_code, object_id, api)

    def _verify_api_by_groups(self, tenant_id: str, app_code: str, group_ids: List[int], api: ManagementAPIEnum):
        """
        ...
        role_related_objects = RoleRelatedObject.objects.filter(
            tenant_id=tenant_id, object_type=RoleRelatedObjectType.GROUP.value, object_id__in=group_ids
        )
        ...
        for role_id in role_ids_set:
            self._verify_api_by_role(tenant_id, app_code, role_id, api)
