...
class OpenResourcePathNode(BaseModel):
    system_id: str = Field(alias="system", default="")  # 默认值为空，兼容早期 API 都允许不传 system
    type: str
    id: str
    name: str = ""

    class Config:
        allow_population_by_field_name = True  # 支持 alias 字段传 system 或 system_id
...
class OpenRelatedResource(BaseModel):
    ...
    attributes: List[
        Any
    ] = []  # 一般情况下，若支持属性，其 API 协议与 PolicyBean 等都一致的，所以不需要额外定义，Any 表示即可
    ...
    def fill_instance_system(self, selections: List[InstanceSelectionBean]):
        """使用实例视图，对所有资源实例路径进行 system 填充"""
        for path in self.paths:
            # 遍历每个实例视图，查询哪些实例视图与 path 是匹配
            ...
            # 判断是否路径上每个节点都有 system，有的话不需要使用实例视图填充
            ...
            # 如果有多个实例视图可以匹配，所有实例视图返回的 system_ids 必须一致
            for selection in match_selections[1:]:
                if system_ids != selection.list_match_path_system_id(path_resource_types):
                    self._raise_match_selection_fail_exception(path)

    def fill_instance_name(self, tenant_id: str):
        """填充资源实例名称
        Note: 这里的填充名字只会对缺名称的进行填充，因为对于授权 API 是说，是可以完全信任接入系统传的名称
        """
        # 必须保证所有资源实例的 system_id 都存在
        assert all(node.system_id != "" for path in self.paths for node in path)

        # 查询资源 Name
        resource_biz = ResourceBiz(tenant_id)
...
class OpenPolicy(BaseModel):
    """
    这是 Open API 对于 授权等涉及到策略相关的对外统一数据结构
    """
    ...
    class Config:
        allow_population_by_field_name = True  # 支持 alias 字段传 system 或 system_id 和 action_id 或 id

    def fill_instance_system(self, tenant_id: str):
        """
        填充 system，由于申请数据里的资源实例可能只有 type 和 id，
        不知道该资源是哪个系统的，所以需要根据权限模型的进行填充
        """
        # 使用模型里的实例视图进行填充
        instance_selection_biz = InstanceSelectionBiz(tenant_id)
    ...
    def fill_instance_name(self, tenant_id: str):
        """填充资源实例名称"""
        for rrt in self.related_resource_types:
            rrt.fill_instance_name(tenant_id)


class OpenCommonTrans:
    """主要是处理 open api 的通用处理，因为所有 open api 都会保持相同的概念协议，所有很多转换逻辑都是通用"""

    def __init__(self, tenant_id: str):
        self.tenant_id = tenant_id
        self.action_check_biz = ActionCheckBiz(tenant_id)

    @staticmethod
    def _to_policies(open_policies: List[OpenPolicy]) -> List[PolicyBean]:
        """批量将 OpenPolicy 转换为 PolicyBean"""
        policies = []
        for open_policy in open_policies:
            related_resource_types = []
            for rrt in open_policy.related_resource_types:
                rrt_dict = rrt.dict()
                # 将多个以 path 方式表示的实例进行分组
                instances = group_paths(rrt_dict["paths"])

                # 若 instances 和 attributes 都为空，则为无限制
                condition = (
