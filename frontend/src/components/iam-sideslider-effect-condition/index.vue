<template>
  <div class="slider-effect-condition-wrap">
    <div
      :class="[
        'effect-item',
        { 'effect-item-empty': timeRangeEmpty }
      ]"
      v-for="(environmentsItem, index) in environmentsData"
      :key="index"
    >
      <bk-select
      ...
      <div class="effect-flex">
        ...
        >
          <bk-option
            v-for="option in effectWeekList"
            :key="option.value"
            :id="option.value"
            :name="$t(`m.info['${option.name}']`)"
          />
        ...
        >
          <bk-option
            v-for="option in effectWeekTimeZone"
            :key="option.value"
            :id="option.value"
            :name="$t(`m.info['${option.name}']`)"
          />
        </bk-select>
        <div class="effect-time-wrap">
          <bk-time-picker
            type="timerange"
          ...
          />
          <p
            v-if="timeRangeEmpty && (!environmentsItem.initTimeRange[0] || !environmentsItem.initTimeRange[1])"
            class="error-tips pt5"
          >
            {{ $t(`m.verify['请选择时间范围']`) }}
          </p>
        </div>
  ...
  export default {
      ...
      data: {
        handler (val) {
          ...
          if (!val.length) {
            ...
            this.environmentsData = cloneDeep(val).map((e) => {
    ...
    methods: {
      handleGetValue () {
        ...
        const environments = cloneDeep(this.environmentsData).map((item) => {
          item.condition = [];
          if (!!item.date.length && item.date.length !== 7) {
            item.date = item.date.reduce(
              (prev, dataItem) => {
          ...
          item.initTimeRange = item.initTimeRange.reduce(
            (p, v) => {
