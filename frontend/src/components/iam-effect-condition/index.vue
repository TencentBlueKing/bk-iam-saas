<template>
  <div class="iam-effect-condition-wrapper">
    <div class="edit-text">
      <bk-button v-if="isDetail" text @click="handleEdit">
        {{ $t(`m.common['编辑']`) }}
      </bk-button>
    </div>
        ...
        handler (val) {
          if (this.isEmpty) {
            ...
            this.curValue = val.reduce((p, v) => {
              ...
              p = `${this.effectType[v.type]}: ${p}${
                weekCopy ? '' : this.$t(`m.info['每天']`)
              }${curValue}${this.effectType[v.type]}${this.$t(`m.info['生效']`)}`;
              return p;
            }, this.$t(`m.info['在']`));
          }
        },
