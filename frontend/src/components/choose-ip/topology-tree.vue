<template>
    <div :class="['iam-topology-tree', { 'bk-has-border-tree': isBorder }]">
        ...
        <div class="render-wrapper" ref="content">
            <div v-for="(item, index) in allData" :key="item.nodeId"
                :class="['node-item', { 'load-more-node': item.type === 'load' }, { 'search-node': item.type === 'search' }, { 'can-hover': item.type === 'node' && !item.loading }]"
                ...
                <template v-if="item.type === 'node'">
                    ...
                    <div class="search-icon" v-if="item.async && item.type === 'node' && !item.loading" @click.stop="handleOpenSearch(item, index)">
                        <Icon bk type="search" />
                    </div>
                </template>
                <template v-else-if="item.type === 'load'">
                    <div class="load-more-wrapper">
                        <div
                            :class="['load-item', { 'loading-more': item.loadingMore }, { 'normal': !item.loadingMore && !isExistNodeLoadMore }, { 'exist-load-more': isExistNodeLoadMore }]"
                            @click="loadMore(item, index)">
                            <template v-if="!item.loadingMore">
                                {{ item.name }}
                            </template>
                            <template v-else>
                                <div class="node-load-more-loading" v-bkloading="{ isLoading: item.loadingMore, opacity: 1, theme: 'primary', size: 'mini' }"></div>
                            </template>
                        </div>
                    </div>
                </template>
                ...
                <template v-else-if="item.type === 'search-loading'">
                    <div class="search-loading-wrapper" v-bkloading="{ isLoading: true, opacity: 1, theme: 'primary', size: 'mini' }"></div>
                </template>
    ...
    export default {
        ...
        methods: {
            ...
            handleOpenSearch (node, index) {
                if ((this.allData[index + 1]
                    && this.allData[index + 1].type === 'search'
                    && this.allData[index + 1].visiable)
             ...
             */
            getNodeStyle (node) {
                const isSameLevelExistSync = this.allData.filter(item => item.level === node.level).some(item => item.type === 'node' && item.async);
