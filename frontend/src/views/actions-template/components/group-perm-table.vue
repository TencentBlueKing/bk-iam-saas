<template>
                    ...
                    <template v-if="resource.resource_groups && resource.resource_groups.length > 0">
                      <div v-for="group in resource.resource_groups" :key="group.id">
                        <div
                          ...
                          v-for="related in group.related_resource_types"
                          :key="related.type"
                        >
                          <span>{{ $t(`m.common['已选择']`) }}</span>
                          <span class="instance-count">{{ formatInstanceCount(related) }}</span>
  ...
  export default {
    ...
    watch: {
      list: {
        handler (value) {
          this.tableList = _.uniqWith(value, _.isEqual);
        },
        immediate: true
      },
      systemId: {
