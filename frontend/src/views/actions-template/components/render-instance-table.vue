<template>
  <div class="template-resource-instance-table-wrapper" v-bkloading="{ isLoading, opacity: 1 }">
    <bk-table
      v-if="!isLoading"
      :data="tableList"
      ...
      <bk-table-column :resizable="false" :label="$t(`m.common['所属系统']`)">
        <template slot-scope="{ row }">
          <span>{{ !!row.isAggregate ? row.system_name : row.detail.system.name }}</span>
        </template>
      </bk-table-column>
      <bk-table-column :resizable="false" :label="$t(`m.common['添加来源']`)">
        <template slot-scope="{ row }">
          <span>{{ !!row.isAggregate ? row.system_name : row.detail.system.name }}</span>
        </template>
      </bk-table-column>
      ...
      <bk-table-column :resizable="false" :label="$t(`m.common['资源实例']`)" :min-width="260">
        <template slot-scope="{ row, $index }">
          ...
          <template v-else>
            <div class="relation-content-wrapper" v-if="!!row.isAggregate">
              <label
                class="resource-type-name"
                v-if="row.aggregateResourceType && row.aggregateResourceType.length === 1">
...
<script>
  ...
  import RenderResource from '@/views/group/components/render-resource';
  import RenderCondition from '@/views/group/components/render-condition';
